...
@rem ##########################################################################
@rem
@rem  grpcServer startup script for Windows
@rem
@rem ##########################################################################
...
@rem Execute grpcServer
"%JAVA_EXE%" %DEFAULT_JVM_OPTS% %JAVA_OPTS% %GRPC_SERVER_OPTS%  -classpath "%CLASSPATH%" java.io.grpc.example.helloworld.HelloWorldClient %*
...
if not ""=="%GRPC_SERVER_EXIT_CONSOLE%" exit %EXIT_CODE%
exit /b %EXIT_CODE%
